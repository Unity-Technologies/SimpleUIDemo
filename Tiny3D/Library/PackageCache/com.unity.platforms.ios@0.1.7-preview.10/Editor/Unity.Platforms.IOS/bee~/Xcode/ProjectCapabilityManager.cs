...
#endif
{
    ...
    public class ProjectCapabilityManager
    {
        ...
        public ProjectCapabilityManager(string pbxProjectPath, string entitlementFilePath, string targetName)
        {
            ...
            {
                throw new Exception(String.Format("Could not find target: {0} in {1}.", targetName, pbxProjectPath));
            }
        }
        ...
        {
            GetOrCreateEntitlementDoc().root[AccessWiFiInformationEntitlements.Key] = new PlistElementBoolean(true);
        ...
        private PlistDocument GetOrCreateEntitlementDoc()
        {
            if (m_Entitlements == null)
            {
                ...
                if (File.Exists(PBXPath.Combine(m_BuildPath, m_EntitlementFilePath)))
                {
                    m_Entitlements.ReadFromFile(PBXPath.Combine(m_BuildPath, m_EntitlementFilePath));
                }
                else
                {
    ...
    internal class AccessWiFiInformationEntitlements
    {
        internal static readonly string Key = "com.apple.developer.networking.wifi-info";
    }
