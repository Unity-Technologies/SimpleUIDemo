...
#endif
{
    ...
    public class PBXProject
    {
        ...
        /// </summary>
        /// <param name="fileGuid">The GUID of the file or folder reference.</param>
        /// <param name="path">The physical path to the new file on the filesystem</param>
        /// <param name="sourceTree">The source tree path is relative to. The [[PBXSourceTree.Group]] tree is not supported.</param>
        public void ReplaceFile(string fileGuid, string path, PBXSourceTree source = PBXSourceTree.Absolute)
        {
            ...
            path = PBXPath.FixSlashes(path);
            var fileRef = FileRefsGet(fileGuid);
            if (fileRef != null)
            {
                fileRef.path = path;
                fileRef.tree = source;
        ...
        /// Name can only be set for shell script and copy file build phases. Type will be returned for
        /// all other phases (same ass [[GetBuildPhaseTypeForTarget(string)]]. Null will be returned if
        /// a phase with the given Guid is not available.
        /// </summary>
        /// <returns>The build phase name if it's is defined.</returns>
        /// <param name="phaseGuid">GUID of the phase.</param>
        public string GetBuildPhaseName(string phaseGuid)
        {
            foreach (var target in nativeTargets.GetObjects())
            {
                if (target.phases.Contains(phaseGuid))
                {
                    var phase = BuildSectionAny(phaseGuid);
                    if (phase is PBXCopyFilesBuildPhaseData)
                    {
                        return ((PBXCopyFilesBuildPhaseData) phase).name;
                    }
                    else if (phase is PBXShellScriptBuildPhaseData)
                    {
                        return ((PBXShellScriptBuildPhaseData) phase).name;
                    }
                    else
                    {
                        return phase.GetPropertyString("isa");
                    }
                }
            }
        ...
        /// Currently these build phase types are defined: PBXSourcesBuildPhase, PBXFrameworksBuildPhase,
        /// PBXResourcesBuildPhase, PBXCopyFilesBuildPhase, PBXShellScriptBuildPhase. </summary>
        /// <returns>The type of the build phase.</returns>
        /// <param name="phaseGuid">GUID of the phase.</param>
        public string GetBuildPhaseType(string phaseGuid)
        {
            ...
            {
                if (target.phases.Contains(phaseGuid))
                    return BuildSectionAny(phaseGuid).GetPropertyString("isa");
            }
        ...
        {
            return m_Data.GetRealPathsRelativeToSourceTree(sourceTree);
        }
        ...
        /// <summary>
        /// Creates a new copy files build phase for given target.
        /// The build phase is inserted at the specified position. All defined build phases can be queried using
        /// [[GetAllBuildPhasesForTarget(string)]].
        /// </summary>
        /// <returns>Returns the GUID of the new phase.</returns>
        /// <param name="index">The position at which to insert the new build phase.
        ///  If it's higher than the total number of build phases the new phase will be added at the end.</param>
        /// <param name="targetGuid">The GUID of the target as returned by [[TargetGuidByName()]].</param>
        /// <param name="name">The name of the phase.</param>
        /// <param name="dstPath">The destination path.</param>
        /// <param name="subfolderSpec">The "subfolder spec". The following usages are known:
        ...
        public string InsertCopyFilesBuildPhase(int index, string targetGuid, string name, string dstPath, string subfolderSpec)
        {
            var phase = PBXCopyFilesBuildPhaseData.Create(name, dstPath, subfolderSpec);
            copyFiles.AddEntry(phase);
            nativeTargets[targetGuid].phases.InsertGUID(index, phase.guid);
            return phase.guid;
        }
        ...
        /// The type and name of each phases can be queried using [[GetBuildPhaseType]]
        /// and [[GetBuildPhaseName]] respectively.
        /// </summary>
        /// <returns>Returns the GUIDs of all build phases for the given target.</returns>
        /// <param name="targetGuid">The GUID of the target as returned by [[TargetGuidByName()]].</param>
        public string[] GetAllBuildPhasesForTarget(string targetGuid)
        {
            return nativeTargets[targetGuid].phases.ToArray();
        }
		
		/// <summary>
		/// Gets the base configuration reference Guid for the specified build configuration.
		/// </summary>
		/// <returns>The GUID of the configuration file being used as the base reference.</returns>
		...
		{
			return buildConfigs[configGuid].baseConfigurationReference;
		}
		...
		/// </summary>
		/// <param name="configGuid">The GUID of the build configuration as returned by [[BuildConfigByName()]].</param>
		/// <param name="baseReference">The GUID of the configuration file to be used as the base reference.</param>
        ...
        /// <summary>
        /// Returns the value of a build property in all build configurations for the specified targets.
        /// Duplicate build properties are ignored. Values for names "LIBRARY_SEARCH_PATHS" and
        /// "FRAMEWORK_SEARCH_PATHS" are quoted if they contain spaces.
        /// </summary>
        /// <param name="targetGuid">The GUID of the target as returned by [[TargetGuidByName()]].</param>
        ...
        public string GetBuildPropertyForAnyConfig(IEnumerable<string> targetGuids, string name)
        {
            foreach (string t in targetGuids)
            {
                string value = GetBuildPropertyForAnyConfig(t, name);
                if (value != null)
                    return value;
            }
        ...
        public string GetBuildPropertyForAnyConfig(string targetGuid, string name)
        {
            foreach (string guid in buildConfigLists[GetConfigListForTarget(targetGuid)].buildConfigs)
            {
                string value = GetBuildPropertyForConfig(guid, name);
                if (value != null)
                    return value;
            }
        ...
        {
            if (buildConfigs[configGuid] != null)
                return buildConfigs[configGuid].GetProperty(name);
            else
                return null;
        }
        ...
        /// The parameters of existing shell script build phase are matched to the arguments of this
        /// function and the GUID of the phase is returned only if a matching build phase is found.
        /// </summary>
        /// <returns>Returns the GUID of the matching phase or null if it does not exist.</returns>
        /// <param name="targetGuid">The GUID of the target as returned by [[TargetGuidByName()]].</param>
        /// <param name="name">The name of the phase.</param>
        /// <param name="shellPath">The shell path.</param>
        /// <param name="shellScript">The shell script.</param>
        public string GetShellScriptBuildPhaseForTarget(string targetGuid, string name, string shellPath, string shellScript)
        {
            return GetShellScriptBuildPhaseForTarget(targetGuid, name, shellPath, shellScript, null);
        }
        ...
        /// <param name="shellPath">The shell path.</param>
        /// <param name="shellScript">The shell script.</param>
        /// <param name="inputPaths">The shell script input dependencies.</param>
        public string GetShellScriptBuildPhaseForTarget(string targetGuid, string name, string shellPath, string shellScript, List<string> inputPaths)
        {
            ...
            foreach (var phaseGuid in target.phases) {
                var phaseAny = BuildSectionAny(phaseGuid);
                if (phaseAny is PBXShellScriptBuildPhaseData)
                {
                    var shellScriptPhase = (PBXShellScriptBuildPhaseData) phaseAny;
                    
                    if (shellScriptPhase.name == name && shellScriptPhase.shellPath == shellPath && 
                        shellScriptPhase.shellScript == shellScript &&
                        ListsEqual(shellScriptPhase.inputPaths, inputPaths))
                    {
                        return phaseGuid;
                    }
                }
        ...
        /// If the target already has a shell script build phase with the same name, dstPath and subfolderSpec 
        /// configured for it, the function returns the existing phase. 
        /// The new phase is placed at the end of the list of build phases configured for the target.
        /// </summary>
        /// <returns>Returns the GUID of the new phase.</returns>
        ...
        {
            return AddShellScriptBuildPhase(targetGuid, name, shellPath, shellScript, null);
        }
        ...
        {
            PBXShellScriptBuildPhaseData shellScriptPhase = PBXShellScriptBuildPhaseData.Create(name, shellPath, shellScript, inputPaths);

            shellScripts.AddEntry(shellScriptPhase);
            nativeTargets[targetGuid].phases.AddGUID(shellScriptPhase.guid);
            return shellScriptPhase.guid;
        }

        /// <summary>
        /// Creates a new shell script build phase for given target.
        /// The build phase is inserted at the specified position. All defined build phases can be queried using
        /// [[GetAllBuildPhasesForTarget(string)]].
        /// </summary>
        /// <returns>Returns the GUID of the new phase.</returns>
        ...
        {
            return InsertShellScriptBuildPhase(index, targetGuid, name, shellPath, shellScript, null);
        }
        ...
        public string InsertShellScriptBuildPhase(int index, string targetGuid, string name, string shellPath, string shellScript, List<string> inputPaths)
        {
            ...
            shellScripts.AddEntry(shellScriptPhase);
            nativeTargets[targetGuid].phases.InsertGUID(index, shellScriptPhase.guid);
            return shellScriptPhase.guid;
        }

        internal void AppendShellScriptBuildPhase(IEnumerable<string> targetGuids, string name, string shellPath, string shellScript)
        {
            AppendShellScriptBuildPhase(targetGuids, name, shellPath, shellScript, null);
        }
